...
const IDEAL_TOTAL_GAMES_PLAYED = 30;  // season matcher will try to get close to
                                      // this value of games played (therefore
                                      // days)
...
module.exports =
    (leagueId, callback) => {
      roundNumber = 1;
      gameNumber = 1;
      nextGameDate = moment().add(1, 'minutes');
      ...
      internalQuery('get', `/leagues/${leagueId}`, {}, league => {
        ...
        internalQuery(
            'get', `/teams?filter={"where": {"id": {"inq": [${arrstr}]}}}`, {},
            teams => {
              // Now we have an array of the teams in teamsArray
              // Generate IDEAL_TOTAL_GAMES_PLAYED rounds
              if (teams.length < 2) {
                logging.warn(
                    'Unable to generate season for league ' + leagueId +
                    '. Not enough teams enrolled.');
                callback(
                    'Must have at least 2 players registered to a league to start a season');
                return;
              }
              var regularGames = [];
              ...
              // Should we go over the amount of games, or under, dpeending
              // which is closer
              if (IDEAL_TOTAL_GAMES_PLAYED - (under * gamesThisSeason) >
                  (over * gamesThisSeason) - IDEAL_TOTAL_GAMES_PLAYED) {
                // Use over
                use = over;
              } else {
                use = under;
              ...
              for (var i = 1; i < use + 1; i++) {
                regularGames =
                    regularGames.concat(shuffle(createRounds(teams, leagueId)));
              }

              // Fetch the season number
              getSeasonNumber(leagueId).then(seasonNumber => {
                // Generate playoffs
                ...
                internalQuery(
                    'post', `/seasons?leagueId=${leagueId}`, {
                      'number': seasonNumber,
                      'leagueId': leagueId,
                    ...
                    createdSeason => {
                      ...
                      logging.event(
                          ...
                          ' generated for league ' + leagueId +
                          '. Refreshing games queue...');
                      callback();
