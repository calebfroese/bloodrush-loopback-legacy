...
function
queueGames() {
  for (var job in nodeSchedule.scheduledJobs) {
    if (job.indexOf(`gameManager.queue.`) !== -1) {
  ...
  internalQuery('get', `/games/allOnDate`, {}, res => {
    ...
    games.forEach(game => {
      ...
      logging.event(
          '>>> ' + game.id + ' @ ' + moment(game.date).format('h:mm:ss a'));
...
function queueGame(gameId, date) {
  nodeSchedule.scheduleJob(`gameManager.queue.${gameId}`, date, function(gId) {
